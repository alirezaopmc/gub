...
const LEGACY_KINDS = new Set<LegacyEventKind>([
  "coin",
  "skull",
  "trump",
  "bonus",
...
function readLoggedAt(o: Record<string, unknown>): number | undefined {
  return typeof o.loggedAt === "number" && Number.isFinite(o.loggedAt)
    ? o.loggedAt
    : undefined;
}
...
function parseRoundEvent(raw: unknown): RoundEvent {
  ...
  if (
    o.type === "alliance" &&
    typeof o.lootPlayerIndex === "number" &&
    typeof o.trickWinnerIndex === "number"
  ) {
    return {
      type: "alliance",
  ...
  if (
    ...
    (o.capturingCard === "pirate" ||
      o.capturingCard === "mermaid" ||
      o.capturingCard === "king") &&
  ...
  if (
    o.type === "fourteenBonus" &&
    typeof o.playerIndex === "number" &&
    isFourteenSuit(o.suit)
  ) {
    return {
      type: "fourteenBonus",
...
export function parsePersistedRounds(rounds: unknown): RoundData[] {
  ...
  return rounds.map((r) => {
    ...
    return {
      ...
      bidsSheetDismissed:
        typeof raw.bidsSheetDismissed === "boolean"
          ? raw.bidsSheetDismissed
          : false,
      players: pl.map((p, playerIndex) => {
        ...
        const fromStored: 1 | -1 | null =
          stored === 1 || stored === -1 ? stored : null;
        return {
          bid:
            typeof pr.bid === "number" || pr.bid === null
              ? (pr.bid as number | null)
              : null,
          won:
            typeof pr.won === "number" || pr.won === null
              ? (pr.won as number | null)
              : null,
          score:
            typeof pr.score === "number" && Number.isFinite(pr.score)
              ? pr.score
              : 0,
          events,
