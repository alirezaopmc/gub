...
function cloneRounds(rounds: RoundData[]): RoundData[] {
  return rounds.map((round) => ({
    ...
    players: round.players.map((p) => ({
      ...
      won: p.won,
      harryGiantBidDelta:
        p.harryGiantBidDelta === 1 || p.harryGiantBidDelta === -1
          ? p.harryGiantBidDelta
          : null,
      score:
...
export type RoundScoreStore = {
  ...
  restoreCurrentRoundFromBidWonModalSnapshot: (
    snapshot: BidWonModalRoundSnapshot,
  ) => void;
  /**
...
export const useRoundScoreStore = create<RoundScoreStore>((set, get) => ({
  ...
  initFromConfig: (config) => {
    ...
    if (
      ...
      const high = Math.max(
        0,
        Math.min(loaded.highestUnlockedRoundIndex, maxI),
      );
      set({
        config: normalized,
  ...
  addEventToPlayerIndices: (playerIndices, event) => {
    ...
    const valid = playerIndices.filter(
      (i) => nextRounds[r]!.players[i] != null,
    );
...
useRoundScoreStore.subscribe((state) => {
  ...
  const { config, currentRoundIndex, highestUnlockedRoundIndex, rounds } =
    state;
  if (!config || rounds.length === 0) return;
  saveRoundData({
    v: 1,
