...
export function loadRoundData(): PersistedRoundState | null {
  ...
  if (typeof playerCount !== "number" || typeof roundCount !== "number")
    return null;
  ...
  for (const round of rounds) {
    if (!round || typeof round !== "object" || !("players" in round))
      return null;
  ...
  if (
    typeof parsed.highestUnlockedRoundIndex !== "number" ||
    !Number.isFinite(parsed.highestUnlockedRoundIndex)
  ) {
    return null;
  }
