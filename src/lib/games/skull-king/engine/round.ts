...
export function createEmptyBids(count: number): PlayerBid[] {
  return Array.from(
    { length: count },
    (): PlayerBid => ({ bid: null, harryGiantDelta: null }),
  );
}
...
export function getExpectedTurnSeat(
  round: RoundState,
  playerCount: number,
): number {
...
function pirateWonWith(
  play: import("./types").PlayedCard,
): import("./types").PirateId | null {
...
export function playCardOnRound(
  ...
  const expectedPlayer =
    round.currentTrick.length === 0
      ...
      : nextSeatAfter(
          round.currentTrick[round.currentTrick.length - 1]!.playerIndex,
          playerCount,
        );
  ...
  const trick: TrickResult =
    lootWin !== null
      ? {
          outcome: { type: "won", winnerIndex: lootWin, plays: currentTrick },
          captures: [],
        }
  ...
  if (trick.outcome.type === "won") {
    tricksWon[trick.outcome.winnerIndex] =
      (tricksWon[trick.outcome.winnerIndex] ?? 0) + 1;
...
export function finalizeRoundScoring(
  round: RoundState,
  config: MatchConfig,
): RoundState {
...
export function submitBidOnRound(
  ...
  const bids = round.bids.map((b, i) =>
    i === playerIndex ? { ...b, bid } : b,
  );
...
export function resolveAbilityOnRound(
  ...
  if (!pending || round.phase !== "ability")
    return { error: "ability_not_pending" };

  let updated: RoundState = {
    ...round,
    pendingAbility: null,
    phase: "playing",
  };
  ...
  switch (resolution.type) {
    ...
    case "bendt": {
      ...
      const undealt = updated.undealt.filter(
        (c) => !drawn.some((d) => d.id === c.id),
      );
      ...
      const hands = updated.hands.map((h, i) =>
        i === pending.winnerIndex ? hand : h,
      );
    ...
    case "rascal":
      updated = {
        ...updated,
        rascalWager: {
          playerIndex: pending.winnerIndex,
  ...
  if (
    updated.completedTricks.length >= updated.handSize &&
    !updated.pendingAbility
  ) {
