...
function tripleMermaidWins(plays: PlayedCard[]): PlayedCard | null {
  if (
    !hasRole(plays, "pirate") ||
    !hasRole(plays, "skull_king") ||
    !hasRole(plays, "mermaid")
  ) {
    return null;
  }
...
function resolveNormalWinner(plays: PlayedCard[]): PlayedCard {
  ...
  for (let i = 1; i < competing.length; i++) {
    ...
    const bestIsSpecial =
      br.role === "pirate" || br.role === "skull_king" || br.role === "mermaid";
    const cIsSpecial =
      cr.role === "pirate" || cr.role === "skull_king" || cr.role === "mermaid";
...
function krakenWhaleOrder(plays: PlayedCard[]): {
  kraken: PlayedCard | null;
  whale: PlayedCard | null;
} {
...
function lootAlliance(
  plays: PlayedCard[],
  winner: PlayedCard,
): { lootPlayerIndex: number; trickWinnerIndex: number } | undefined {
  ...
  return {
    lootPlayerIndex: lootPlay.playerIndex,
    trickWinnerIndex: winner.playerIndex,
  };
}
...
export function resolveTrick(
  ...
  if (kraken && whale) {
    ...
    if (second.card.def.kind === "kraken") {
      const wouldBe = resolveNormalWinner(
        sorted.filter(
      ...
      outcome = {
        type: "destroyed",
        wouldBeWinnerIndex: wouldBe.playerIndex,
        plays: sorted,
      };
    } else {
    ...
    const wouldBe = resolveNormalWinner(
      sorted.filter((p) => p.card.def.kind !== "kraken"),
    );
    outcome = {
      type: "destroyed",
      wouldBeWinnerIndex: wouldBe.playerIndex,
  ...
  const captures =
    outcome.type === "won" && winner ? detectCaptures(sorted, winner) : [];
