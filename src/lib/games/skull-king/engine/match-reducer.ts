...
import { BASE_DECK_CONFIG, type DeckConfig, FULL_DECK_CONFIG } from "./deck";
import {
  createRoundState,
...
export function deckConfigFromArtifacts(
...
export function reconcileMatchToSeatedPlayers(
  state: MatchState,
  seatedCount: number,
): MatchState {
...
export function confirmRound(state: MatchState): MatchState {
  ...
  const roundHistory = [
    ...state.roundHistory,
    snapshotRoundToHistory(state, scored),
  ];
