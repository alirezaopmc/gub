...
function clampConfigFromWire(
  input: StoredGameConfig,
): SkullKingGameConfig | null {
  if (!Array.isArray(input.players) || !Array.isArray(input.roundsSchema))
    return null;
  ...
  if (namedCount < MIN_CREW_PLAYERS || namedCount > MAX_CREW_PLAYERS)
    return null;
 ...
 */
export function getDefaultArtifactOptionsForNewGame(): Record<
  Artifacts,
  boolean
> {
