...
function createInitialSetupSlice(): Pick<
  SkullKingStore,
  "players" | "roundsSchema" | "options"
> {
  return {
...
export const useSkullKingStore = create<SkullKingStore>((set, get) => ({
  ...
  hydrateFromGameConfig: (config) => {
    ...
    set({
      players: config.players.map((name) =>
        createCrewRow(clampCrewPlayerName(name)),
      ),
  ...
  setCrewCount: (count) =>
    set((s) => {
      const n = Math.min(
        ...
        Math.max(
          MIN_CREW_PLAYERS,
          Math.floor(Number.isFinite(count) ? count : MIN_CREW_PLAYERS),
        ),
      );
      if (n === s.players.length) return s;
      if (n > s.players.length) {
        const extra = Array.from({ length: n - s.players.length }, () =>
          createCrewRow(""),
        );
  ...
  reorderPlayers: (fromIndex, toIndex) =>
    set((s) => {
      ...
      if (fromIndex >= s.players.length || toIndex >= s.players.length)
        return s;
