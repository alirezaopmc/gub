...
describe("card-ref-ids", () => {
  ...
  it("resolves generic character defaults", () => {
    expect(resolveCardRefId("pirate").def).toEqual({
      kind: "pirate",
      pirate: "rosie",
    });
    expect(resolveCardRefId("mermaid").def).toEqual({
      kind: "mermaid",
      mermaid: "alyra",
    });
  });
  ...
  it("resolves specific pirate and mermaid variants", () => {
    expect(resolveCardRefId("pirate-rascal").def).toEqual({
      kind: "pirate",
      pirate: "rascal",
    });
    expect(resolveCardRefId("mermaid-sirena").def).toEqual({
      kind: "mermaid",
      mermaid: "sirena",
    });
  });
  ...
  it("maps ids to card image paths", () => {
    expect(resolveCardRefImagePath("kraken")).toBe(
      "/games/skull-king/cards/kraken.png",
    );
