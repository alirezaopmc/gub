...
import {
  listDocsBySource,
  sourceFileExists,
} from "@/lib/docs/list-docs-by-source";
...
function assertSnippetInDoc(
  ...
  expect(
    content,
    `${relativePath} missing "${snippet}" (${context})`,
  ).toContain(snippet);
}
...
describe("score-rules docs drift", () => {
  ...
  it("round breakdown fixtures match computeRoundScoreBreakdown", () => {
    for (const fixture of ROUND_BREAKDOWN_FIXTURES) {
      ...
      expect(breakdown.main, `${fixture.label} main`).toBe(
        fixture.expectedMain,
      );
      expect(breakdown.bonus, `${fixture.label} bonus`).toBe(
        fixture.expectedBonus,
      );
      expect(breakdown.total, `${fixture.label} total`).toBe(
  ...
  it("bonus rates match score-rules functions", () => {
    ...
    expect(scoreFourteenBonus([fourteenEvents[0]])).toBe(
      SCORING_BONUS_RATES.fourteenStandard,
    );
    expect(scoreFourteenBonus([fourteenEvents[1]])).toBe(
      SCORING_BONUS_RATES.fourteenBlack,
    );
    ...
    expect(scoreAlliance(0, allyA, [allyA, allyB], 5)).toBe(
      SCORING_BONUS_RATES.alliance,
    );
