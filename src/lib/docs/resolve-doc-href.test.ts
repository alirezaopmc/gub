...
import {
  docsRelativePathToRoute,
  resolveDocHref,
} from "@/lib/docs/resolve-doc-href";

describe("docsRelativePathToRoute", () => {
  it("maps rules paths", () => {
    expect(
      docsRelativePathToRoute("games/skull-king/rules/06-scoring.md"),
    ).toBe("/games/skull-king/docs/rules/06-scoring");
  });
  ...
  it("maps reference paths", () => {
    ...
    ).toBe("/games/skull-king/docs/reference/scoring-quick-ref");
  });
});
...
describe("resolveDocHref", () => {
  ...
  it("passes through absolute app paths", () => {
    expect(resolveDocHref(fromRules, "/games/skull-king/play")).toBe(
      "/games/skull-king/play",
    );
  });
  ...
  it("maps app guide links", () => {
    expect(resolveDocHref(fromRules, "../app/play.md")).toBe(
      "/games/skull-king/docs/play",
    );
  });
  ...
  it("maps reference links", () => {
    expect(
      resolveDocHref(
        ...
        "../reference/scoring-quick-ref.md",
      ),
    ).toBe("/games/skull-king/docs/reference/scoring-quick-ref");
  });
});
