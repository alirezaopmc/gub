...
export type EphemeralToastVariant =
  | "success"
  | "info"
  | "warning"
  | "destructive";
...
export function EphemeralToast({
  ...
  const [lifecycle, setLifecycle] = React.useState<ToastLifecycle>({
    status: "empty",
  });
  ...
  React.useEffect(() => {
    ...
    setLifecycle((prev) =>
      prev.status === "shown"
        ? { status: "leaving", msg: prev.msg, v: prev.v }
        : prev,
    );
  }, [message, variant]);
  ...
  React.useEffect(() => {
    ...
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      const id = window.requestAnimationFrame(() =>
        setLifecycle((prev) =>
          prev.status === "leaving" ? { status: "empty" } : prev,
        ),
      );
  ...
  const onToastPanelAnimationEnd = React.useCallback(
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        <p className="m-0 min-w-0 flex-1 text-left text-sm leading-snug text-foreground">
          {msg}
        </p>
      </div>
    </div>,
