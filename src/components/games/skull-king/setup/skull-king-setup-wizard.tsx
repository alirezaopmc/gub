...
function shouldIgnoreEnterForWizardAdvance(
...
function SkullKingSetupWizardInner() {
  ...
  React.useEffect(() => {
    ...
    router.replace(buildSetupHref(pathname, searchParams, step), {
      scroll: false,
    });
  ...
  React.useLayoutEffect(() => {
    const focusPrimary = () => {
      if (step === 1) {
        document
          .getElementById(WIZARD_STEP_FOCUS_ID.crewFirstNameInput)
          ?.focus();
        return;
      }
      if (step === 2) {
        document
          .getElementById(WIZARD_STEP_FOCUS_ID.navigationRoundSchemaInput)
          ?.focus();
        return;
      }
      if (step === 4) {
        const confirmEl = document.getElementById(
          WIZARD_STEP_FOCUS_ID.reviewConfirm,
        );
  ...
  const goBack = () => {
    if (step <= 1) return;
    router.push(buildSetupHref(pathname, searchParams, step - 1), {
      scroll: false,
    });
  };
  ...
  const wizardFooter = showWizardFooter ? (
