...
export function CrewManifesto() {
  ...
  const errorsVisible =
    interaction === null ? true : interaction.crewNameFieldsBlurredOnce;
  ...
  const dupHintMessage =
    dupGroups.length === 0 ? undefined : formatDuplicateCrewMessage(dupGroups);
              ...
              {players.map((row, index) => (
                <SortableCrewMemberRow
                  ...
                  duplicateHintMessage={
                    errorsVisible && duplicateIndices.has(index)
                      ? dupHintMessage
                      : undefined
                  }
                  ...
                  onEnterInNameField={() => {
                    if (index < players.length - 1) {
                      document
                        .getElementById(`crew-name-${index + 1}`)
                        ?.focus();
                    } else {
                      requestAdvance?.();
                    }
