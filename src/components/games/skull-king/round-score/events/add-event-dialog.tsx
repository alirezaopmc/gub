  ...
  switch (step.t) {
    ...
    case "pir_v": {
      if (step.pirate === "harry")
        return "Adjust the bid by +1 or −1 (applies to the bid cell; B+1 / B−1).";
...
export function EventsDialog({
  ...
  const addEventToPlayerIndices = useRoundScoreStore(
    (s) => s.addEventToPlayerIndices,
  );
  const applyHarryGiantBidDelta = useRoundScoreStore(
    (s) => s.applyHarryGiantBidDelta,
  );
  ...
  const eventLog = React.useMemo(
    () =>
      currentRound != null
        ? getRoundEventLogEntries(currentRound, players)
        : [],
  ...
  React.useEffect(() => {
    if (open) {
      if (!dialogWasOpenRef.current) {
        const hubToUse =
          defaultHub === "add" && !canAddEvent ? "list" : defaultHub;
        setHub(hubToUse);
        setStep({ t: "menu" });
      }
    } else {
  ...
  const body = () => {
    ...
    if (step.t === "al_loot") {
      return renderPlayerList({
        onPick: (loot) => setStep({ t: "al_trick", loot }),
      });
    }
    if (step.t === "al_trick") {
      return renderPlayerList({
    ...
    if (step.t === "pir_o") {
      return renderPlayerList({
        onPick: (owner) => setStep({ t: "pir_k", owner }),
      });
    }
    if (step.t === "pir_k") {
      return (
        <div className="flex flex-col gap-2">
          ...
          >
            ...
            <p>
              Change your bid by +1, −1, or leave it the same (after winning a
              trick with this pirate).
            </p>
          </Button>
          <Button
            type="button"
          ...
          >
            ...
            <p>
              Bet 10 or 20 on making your bid; win or lose those points (after
              winning a trick with this pirate).
            </p>
          </Button>
        </div>
      );
    ...
    if (step.t === "cap_c") {
      return renderPlayerList({
        onPick: (c) => setStep({ t: "cap_t", capturer: c }),
      });
    }
    if (step.t === "cap_t") {
      const opts = [
    ...
    if (step.t === "fb_p") {
      return renderPlayerList({
        onPick: (p) => setStep({ t: "fb_s", player: p }),
      });
    }
    if (step.t === "fb_s") {
      return (
      ...
      {EVENT_KIND_ORDER.map((kind) => {
        ...
        return (
          ...
          >
            ...
            <span className={addStyles.kindTileLabel}>
              {addableEventLabel(kind)}
            </span>
          </Button>
        );
      })}
  ...
  const listDesc =
    "Chronological log for this round (oldest first when times are available).";
  const addDesc = stepSubtitle(step);
  const describedBy = atEventHub
    ? "events-hub-desc"
    : addDesc != null
      ? "add-event-desc"
      : undefined;
