...
export function BidWonDialog({
  ...
  const allBidsSet =
    !config || rounds.length === 0
      ? true
      : areAllNamedPlayersBidsSet(config, rounds[currentRoundIndex]);

  const allWonSet =
    !config || rounds.length === 0
      ? true
      : areAllNamedPlayersWonSet(config, rounds[currentRoundIndex]);
  ...
  const doneDisabledTitle = !doneDisabled
    ...
    : [
        ...
        editableWon && !allWonSet
          ? "Enter tricks won for every named player"
          : null,
      ]
        .filter(Boolean)
        .join(" · ");
